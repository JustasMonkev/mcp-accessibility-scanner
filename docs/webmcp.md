...
After navigating to a page, request MCP `tools/list`. A supported `document.modelContext` or `navigator.modelContext` API contributes dynamic tools alongside the built-in tools. The server does not install a polyfill, enable browser flags, launch a standalone browser solely to list tools, or upgrade Playwright. Non-isolated CDP requests reconnect to their configured shared browser when listing or calling page tools. Extension mode attaches only for a tool call, because attaching waits for the user to connect the extension; its page tools are listed once a call has connected the browser. Page tools are best-effort in a listing: if the shared browser cannot be reached, the built-in tools are still returned and the failure is logged. Cancelling such a request releases it at once; an attachment still in progress never opens a page for it afterwards and is released with the request's browser context.
